...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check_sql = "SELECT id FROM appointments WHERE id = ? AND donor_id = ? AND status = 'approved'";
    ...
    $sql = "UPDATE appointments 
            ...
            WHERE id = ? AND donor_id = ? AND status = 'approved'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssii", $appointment_date, $appointment_time, $appointment_id, $_SESSION['user_id']);

    if ($stmt->execute()) {
        $_SESSION['success'] = "Appointment rescheduled successfully. Waiting for blood bank approval.";
    } else {
        $_SESSION['error'] = "Error rescheduling appointment: " . $conn->error;
    }
